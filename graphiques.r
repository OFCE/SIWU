...
gdis_feb <- map(1:3, ~{
  ...
  graph2png(gg, file="inflation_coicop_wiu_{.x}" |> glue::glue(), dpi=600)})
...
gdis_1y <- map(1:3, ~{
  gg <- ggplot(inf_sorted |> filter(d==dmax$dmax, coicop_digit==3, ref==since_1y), 
    ...
    ggforce::facet_wrap_paginate(vars(geo_f), nrow=3, ncol=3, page = .x)
  graph2png(gg, file="inflation_coicop_1y_{.x}" |> glue::glue(), dpi=600)})
...
g_1y <- map(1:3, ~{
  ...
  graph2png(gg, file="depuis_1y_{.x}" |> glue(), width = pgn_w, height = pgn_h, dpi=600)})
...
g_wiu <- map(1:3, ~{
  gg <- ggplot(inf_sorted |> filter(d==dmax$dmax, ref==since_wiu)) +
    ...
    ggrepel::geom_text_repel(
      ...
      max.overlaps = 100, ylim=c(NA, 0), show.legend=FALSE)+ 
    facet_wrap_paginate(vars(geo_f),  nrow=3, ncol=3, page = .x)
  graph2png(gg, file="depuis_wiu_{.x}" |> glue::glue(), width = pgn_w, height = pgn_h, dpi=600)
  graph2png(gg, file="depuis_wiu_(pres)_{.x}" |> glue::glue(), width = 26, height = 18, dpi=600)})
...
graph2png(g_wiu_NLD, file="depuis_wiu_NLD" |> glue::glue(), width = pgn_w, height = pgn_h, dpi=600)
...
gql1 <- imap(1:3, ~{
  ...
  graph2png(gg, file = "coicop_l1_wiu_{.x}" |> glue::glue(), width = pgn_w, height = pgn_h, dpi=600)
})
...
gql1_1y <- imap(1:3, ~{
  gg <- ggplot(outcome_sorted |> filter(d==dmax_o$dmax, ref==since_1y) ) +
    ...
    labs(title="Impact on income per quintile of income since war in Ukraine",
         ...
         caption=str_c("Sources: Eurostat HICP, consumption per quintile (hbs_str_t223), consumtion per cociop (hbs_str_t211),",
                       "income and expenditure per quintile (icw_res_02)", sep="\n"))
  graph2png(gg, file = "coicop_l1_1y_{.x}" |> glue::glue(), width = pgn_w, height = pgn_h, dpi=600)
})
...
gql1_nld <- ggplot(outcome_sorted |> filter(d==dmax_o$dmax, ref==since_wiu, geo=="NL")) +
  ...
  labs(title="Impact on income per quintile of income since war in Ukraine ofr The Netherlands",
       ...
       caption=str_c("Sources: Eurostat HICP, consumption per quintile (hbs_str_t223), consumtion per cociop (hbs_str_t211),",
                     "income and expenditure per quintile (icw_res_02)", sep="\n"))
graph2png(gql1_nld, file = "coicop_l1_wiu_NLD" |> glue::glue(), width = pgn_w, height = pgn_h, dpi=600)
...
save(gdis_feb, gdis_1y, g_1y, g_wiu, gql1, cts, str_wiu, str_1y, dmax, dmax_o, dmaxx, dmaxx_o, since_1y, since_wiu, file="data/sorties.rdata")
