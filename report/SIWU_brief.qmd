...
date: "2022-11-17"
abstract: |
    *Using data on HICP per COICOP and quintile, we calculate for EU member states the impact on income of households of price evolution since the invasion of Ukraine by Russia. Most of the loss in income is due to energy price increases. In some countries, food prices are also an important factor. Code and data used in the note are available at <http://github.com/ofce/siwu>. Updated with the latest HICP release given by Eurostat, on `r Sys.Date()`.*
format:
  html: 
    toc: true
    toc-location: left 
...
last_update_data <- eurostat::get_eurostat_toc() %>%
...
economic and social impacts of the war in Ukraine (SIWU). To that end, we shall focus on the evolution
of Europeans' purchasing power in the short run. We use the latest data on inflation compiled by Eurostat,
last updated on `r format(last_update_data, '%d %B, %Y')`.
...
gas, certain cereals or oil seeds, nitrogen fertilizers and other products used in industry. Often, there
are substitutes (of origin or nature) for these products, but the rigidity of supply
...
extending to gas supplies has led to an amplification of price movements. Evidently, an embargo would amplify it
...
-   The first one is government interventions to compensate price increases when not directly
    acting on prices. In particular, exceptional checks, increases in allowances or income are not being accounted
    for. Data provision on this matter, for example by Eurostat, in a standardized format, would be very useful.

-   We don't take into account expected increases in future prices due to automatic indexation mechanisms.
    For instance, in some countries, rents are re-evaluated on an index which is calculated using the
    prices of certain goods or services. Other goods and services can also be indexed on inflation, such as 
    tolls on highways. Once again, systematic data collection effects could be undertaken,
    for example by the European Central Bank, because automatic indexations of goods and services are very
    important for understanding inflation developments, and they can potentially be very heterogeneous across 
    countries.

-   We don't take into account increases in wages or other components of income that could increase in
    the future due to price increases. Some of those indexations are explicit in private contracts or
    ...
    purchasing power and redistributive effects caused by increases in prices, being a general
